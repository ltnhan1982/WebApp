...
public class UserService {
    ...
    public void showDoctors(Model model) {
        ...
        model.addAttribute("doctors", this.getDoctorInfo(role.getUsers()));
    }

    private List<DoctorDTO> getDoctorInfo(Set<User> doctors) {
    ...
    public boolean checkPatientExist(String userName) {
        User user = userRepository.findByUsername(userName);
        if (ObjectUtils.isEmpty(user)) {
            return false;
        } else {
